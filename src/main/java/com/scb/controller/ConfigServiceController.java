...
public class ConfigServiceController {
	...
	public ResponseEntity<Void> addProcessFlowSequence(@RequestBody ProcessFlowSequence processflowsequence,
	...
	public ResponseEntity<Void> addServiceDetail(@RequestBody ServiceDetail servicedetail,
			UriComponentsBuilder builder) {
	...
	@RequestMapping(value = "/ProcessFlowSequenceByName", method = RequestMethod.POST, produces = { "application/xml",
			"application/json" })
	public ResponseEntity<ResponseMessage> getProcessFlowSequenceByName(@RequestBody RequestData requestData) {
		...
		AuditLog auditLog = commonMethods.getAuditLog(requestData, "RECEIVED", "Services retrieval request initiated");
		...
		if (responseMessage.getResponseCode() != 200) {
			auditLog = commonMethods.getAuditLog(requestData, "FAILED",
					"Failed retrieve services for transaction type: " + requestData.getTransactionType());
		} else {
			auditLog = commonMethods.getAuditLog(requestData, "COMPLETED",
					"Retrieved services for transaction type: " + requestData.getTransactionType() + " successfully");
		}

		responseAuditLog = internalApiInvoker.auditLogApiCall(auditLog);
		// log.info("Process Flow Sequences for " + requestData.getTransactionType() + "
		// received: " + processflowsequence);
	...
	public ResponseEntity<List<ProcessFlowSequence>> getProcessFlowSequenceById(
	...
	public ResponseEntity<ProcessFlowSequence> getProcessFlowSequenceByCompositeId(
			@PathVariable("processId") long processId, @PathVariable("serviceId") long serviceId) {
		log.info(" Get Process Flow Sequence received: " + processId);
	...
	public ResponseEntity<ProcessFlowSequence> updateProcessFlowSequence(
			@RequestBody ProcessFlowSequence processflowsequence) {
